    ...
    static class Program
    {
        ...
        static void Main(string[] args)
        {
            if(IsNet45OrNewer())
            {
                ...
                {
                    MessageBox.Show("Das Update wurde aufgrund eines Fehlers (" + ex.ToString() + ") abgebrochen!", "Update abgebrochen", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    Application.Exit();
                }
            }
            else
        ...
        public static bool IsNet45OrNewer()
        {
            ...
            using (RegistryKey ndpKey = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry32).OpenSubKey(subkey))
            {
                ...
                {
                    // test NET version
                    switch (CheckNetVersion((int)ndpKey.GetValue("Release")))
                    {
                        case "4.7.1+":
                            return true;
                        case "4.7":
